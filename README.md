...
A complete, production-grade DevSecOps platform deployed on-premises using **VMware vSphere / vCenter** as the hypervisor, **RKE2** as the CNCF-conformant Kubernetes distribution, and **ArgoCD** for GitOps-driven continuous delivery. Every component -- from the bare-metal VM provisioning through Terraform to the application CI/CD pipeline running on GitLab -- is defined as code in this repository.
...
This repository contains the Infrastructure-as-Code (IaC) and GitOps manifests for a fully self-hosted DevSecOps platform. The platform is designed to demonstrate an end-to-end secure software delivery lifecycle on a single on-premises VMware vSphere / vCenter cluster.

**What this platform provides:**

- **Infrastructure Provisioning** -- Terraform provisions Ubuntu 22.04 virtual machines on VMware vSphere / vCenter with static IP addressing and cloud-init based RKE2 bootstrap.
...
```mermaid
graph TB
    subgraph vSphere["VMware vSphere / vCenter (10.0.0.1)"]
    ...
    style VSphere fill:#1a1a2e,color:#fff
...
flowchart TB
    subgraph HW["Bare Metal"]
        vSphere["VMware vSphere / vCenter<br/>10.0.0.1"]
    end
    ...
    VSphere -->|"provisions VMs"| Mgmt & Cluster
...
```mermaid
flowchart TB
    subgraph HW["VMware vSphere / vCenter (10.0.0.1)<br/>80 vCPU / 756 GB RAM"]
...
|---|---|---|---|
| **VMware vSphere / vCenter** | Bare-metal hyperconverged infrastructure | Terraform provider >= 0.6.0 | N/A (hypervisor) |
...
**VMware vSphere / vCenter** -- An open-source enterprise virtualization platform. Provides VM management, storage, and networking via vSphere. Used as the foundation layer to host all VMs that form the RKE2 clusters, eliminating the need for separate hypervisor and storage solutions.

**Terraform** -- A declarative Infrastructure-as-Code (IaC) tool by HashiCorp. Provisions and manages vSphere VMs, folders, templates, and networks through the vSphere Terraform provider. All infrastructure is defined in `.tf` files, enabling reproducible, version-controlled deployments.
...
terraform apply  # Creates the VM on vSphere
```
...
| **VMware vSphere / vCenter** | https://10.0.0.1 | `admin` | `CHANGE_ME_USER_PASSWORD$$$` |
...
| Application | URL | Port | Notes |
|---|---|---|---|
| VMware vSphere / vCenter | https://10.0.0.1 | 443 | vCenter management UI (direct access) |
...
- VMware vSphere / vCenter cluster running and accessible at `administrator@vsphere.local`
- vCenter connection saved to `vcenter.homelab.local`
...
# Initialize Terraform (downloads the vSphere provider)
terraform init

# Review the plan
terraform plan

# Apply -- creates 1 CP + 4 worker VMs on vSphere
terraform apply
```
...
|---|---|---|---|
| `vsphere_server` | string | `vcenter.homelab.local` | vCenter Server FQDN or IP |
| `vsphere_user` | string | `administrator@vsphere.local` | vCenter username |
...
| `vm_namespace` | string | `default` | vSphere namespace where VMs are created |
...
|-- versions.tf                            # Terraform >= 1.5.0, vSphere provider >= 0.6.0
