...
A complete, production-grade DevSecOps platform deployed on-premises using **VMware vSphere** as the hypervisor, **RKE2** as the CNCF-conformant Kubernetes distribution, and **ArgoCD** for GitOps-driven continuous delivery. Every component -- from the bare-metal VM provisioning through Terraform to the application CI/CD pipeline running on GitLab -- is defined as code in this repository.
...
This repository contains the Infrastructure-as-Code (IaC) and GitOps manifests for a fully self-hosted DevSecOps platform. The platform is designed to demonstrate an end-to-end secure software delivery lifecycle on a single on-premises VMware vSphere cluster.

**What this platform provides:**

- **Infrastructure Provisioning** -- Terraform provisions Ubuntu 22.04 virtual machines on VMware vSphere with static IP addressing and cloud-init based RKE2 bootstrap.
...
```mermaid
graph TB
    subgraph Hypervisor["VMware vSphere"]
        direction TB
    ...
    Terraform["Terraform<br/>vsphere provider"]
...
|---|---|---|---|
| **VMware vSphere** | Bare-metal hyperconverged infrastructure | Terraform provider >= 0.6.0 | N/A (hypervisor) |
...
**VMware vSphere** -- An open-source hyperconverged infrastructure (HCI) platform built on Kubernetes. Provides VM management, storage, and networking on bare-metal servers. Used as the foundation layer to host all VMs that form the RKE2 clusters, eliminating the need for separate hypervisor and storage solutions.
...
| **VMware vSphere** | https://10.0.0.1 | `admin` | `CHANGE_ME_USER_PASSWORD$$$` |
...
| Application | URL | Port | Notes |
|---|---|---|---|
| VMware vSphere | https://10.0.0.1 | 443 | Hypervisor management UI (direct access, not through MetalLB) |
...
- VMware vSphere cluster running and accessible at `https://10.0.0.1`
