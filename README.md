...
```mermaid
graph TB
    subgraph Harvester["VMware vSphere / vCenter (10.0.0.1)"]
...
flowchart TB
    subgraph HW["Bare Metal"]
        Harvester["VMware vSphere / vCenter<br/>10.0.0.1"]
    end
...
**VMware vSphere / vCenter** -- An open-source hyperconverged infrastructure (HCI) platform built on Kubernetes. Provides VM management, storage, and networking on bare-metal servers. Used as the foundation layer to host all VMs that form the RKE2 clusters, eliminating the need for separate hypervisor and storage solutions.
...
| Application | URL | Port | Notes |
|---|---|---|---|
| VMware vSphere / vCenter | https://10.0.0.1 | 443 | Hypervisor management UI (direct access, not through MetalLB) |
...
- VMware vSphere / vCenter cluster running and accessible at `https://10.0.0.1`
